...
// Mirrors framed_server, but reads each message body via read_exact_into()
// straight into its own destination buffer instead of read_exact()'s
// gather-into-this-reader's-own-buffer -- exercises read_exact_into()'s
// handling of a previous call's overshoot tail (bytes left in FramedReader's
// buffer because a chunk delivered more than that call needed) getting
// consumed on a *later* call, alongside genuinely fresh chunks.
easyio::Task<void> framed_server_into(
    easyio::Queue& queue, int listen_fd, std::vector<std::string>* out, bool multishot = true) {
    int client_fd = co_await queue.accept(listen_fd);
    auto stream = queue.recv_stream(client_fd, 4096, 4, multishot);
    easyio::FramedReader reader(*stream);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        co_await reader.read_exact_into(body.data(), len);
...
TEST_P(EasyioTest, FramedReaderExactBoundariesInto) {
    auto queue = easyio::Queue::create(GetParam(), 32);
    ...
    run_pair(
        *queue, framed_server_into(*queue, listen_fd, &received),
        framed_client(*queue, client_fd, addr, expected));
